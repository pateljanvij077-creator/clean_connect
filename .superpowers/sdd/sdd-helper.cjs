...
if (!command) {
  console.error("Usage: node sdd-helper.cjs [task-brief|review-package] [args]");
  process.exit(1);
}
...
if (command === 'task-brief') {
  if (args.length < 3) {
    console.error("Usage: node sdd-helper.cjs task-brief PLAN_FILE TASK_NUMBER");
    process.exit(1);
  }
  ...
  if (args.length < 3) {
    console.error("Usage: node sdd-helper.cjs review-package BASE HEAD");
    process.exit(1);
  }
  const base = args[1];
  const head = args[2];
